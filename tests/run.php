...
foreach(['reciprocal_link_enabled','reciprocal_rss_enabled','allocation_type','reciprocal_rss_distribution_history','reciprocal_rss_daily_displays'] as $needle){if(!str_contains($migration,$needle)){fwrite(STDERR,"FAIL reciprocal RSS migration missing: {$needle}\n");$failed++;}}
$distribution=(string)file_get_contents(dirname(__DIR__).'/src/DistributionService.php');foreach(['is_suspicious=0','priority_120','priority_150','priority_200','reciprocal_link_id'] as $needle){if(!str_contains($distribution,$needle)){fwrite(STDERR,"FAIL reciprocal distribution missing: {$needle}\n");$failed++;}}
