...
if ($report === 'traffic') {
    ...
    $sessionsStmt = $db->prepare("SELECT COUNT(DISTINCT session_hash) FROM raw_events WHERE site_id=? AND event_type='pageview' AND is_bot=0 AND is_suspicious=0 AND DATE(occurred_at)=CURDATE() AND session_hash IS NOT NULL");
...
if ($report === 'pages') {
    $stmt = $db->prepare("SELECT normalized_page_url,COUNT(*) pv,COUNT(DISTINCT visitor_hash) uu FROM raw_events WHERE site_id=? AND event_type='pageview' AND is_bot=0 AND is_suspicious=0 AND occurred_at>=NOW()-INTERVAL 30 DAY GROUP BY normalized_page_url ORDER BY pv DESC LIMIT 100");
    $stmt->execute([$siteId]);
...
if ($report === 'audience') {
    $stmt = $db->prepare("SELECT COALESCE(device,'不明') device,COUNT(*) views FROM raw_events WHERE site_id=? AND event_type='pageview' AND is_bot=0 AND is_suspicious=0 AND occurred_at>=NOW()-INTERVAL 30 DAY GROUP BY device ORDER BY views DESC");
    $stmt->execute([$siteId]);
