...
try {
    $result = (new SiteMetadataService())->fetchMetadata((string) ($_POST['url'] ?? ''));
    echo json_encode(['ok'=>true,'name'=>$result['name'],'source'=>$result['source'],'feeds'=>$result['feeds']], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
} catch (Throwable $e) {
