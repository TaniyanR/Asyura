...
header('X-Content-Type-Options: nosniff');header('X-Robots-Tag: noindex,nofollow,noarchive');header("Content-Security-Policy: default-src 'none'; img-src https: data:; style-src 'unsafe-inline'; script-src 'nonce-{$nonce}'; frame-ancestors *; base-uri 'none'");
...
?><!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><style>html,body{margin:0;padding:0;background:transparent}*{box-sizing:border-box}<?= $widget['custom_css'] ?></style></head><body><div class="asyura-<?= e($type) ?>" data-asyura-widget="<?= (int)$widget['id'] ?>"><?= $renderer->render($widget,$items) ?></div><script nonce="<?= e($nonce) ?>" src="<?= e(app_url('assets/widget-resize.js')) ?>"></script></body></html>
