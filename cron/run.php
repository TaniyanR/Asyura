...
use Asyura\ReciprocalHealthService;
...
try{
    $rss=(new RssService($db))->fetchDue();$linkHealth=(new ReciprocalHealthService($db))->checkDue();$distribution=new DistributionService($db);$siteIds=$db->query('SELECT id FROM sites WHERE active=1 AND rss_enabled=1')->fetchAll(PDO::FETCH_COLUMN);foreach($siteIds as $siteId)$distribution->calculate((int)$siteId);
    ...
    $message='RSS成功 '.$rss['success'].' / 失敗 '.$rss['failed'].' / 相互先確認 '.$linkHealth['checked'].' / 配分 '.count($siteIds).'サイト';$db->prepare("UPDATE cron_runs SET status='success',message=?,finished_at=NOW() WHERE id=?")->execute([$message,$runId]);$out=['ok'=>true,'message'=>$message];
