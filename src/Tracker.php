...
final class Tracker
{
    ...
    public function record(array $site,array $payload):void
    {
        ...
        $ua=mb_substr((string)($_SERVER['HTTP_USER_AGENT']??''),0,500);$classification=TrafficClassifier::classify($ua,$payload);$isBot=(bool)$classification['is_bot'];$isSuspicious=(bool)$classification['is_suspicious'];
        if($isSuspicious)$this->logSecurity((int)$site['id'],'automation_detected','warning',(string)$classification['reason'],(string)($_SERVER['HTTP_ORIGIN']??''));
        ...
        if($eventType==='engagement'){if(!$isSuspicious)$this->recordEngagement((int)$site['id'],$pageviewId,$sessionHash,$payload);return;}
