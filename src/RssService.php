...
final class RssService
{
    ...
    {
        $feeds=$this->db->query("SELECT f.* FROM rss_feeds f JOIN sites s ON s.id=f.site_id WHERE f.active=1 AND s.active=1 AND (f.reciprocal_link_id IS NULL OR EXISTS (SELECT 1 FROM reciprocal_links l WHERE l.id=f.reciprocal_link_id AND l.site_id=f.site_id AND l.status='approved' AND (l.reciprocal_link_enabled=1 OR l.reciprocal_rss_enabled=1) AND l.is_excluded=0 AND l.allocation_type<>'excluded')) AND (f.last_fetched_at IS NULL OR f.last_fetched_at<=NOW()-INTERVAL 25 MINUTE)")->fetchAll();
