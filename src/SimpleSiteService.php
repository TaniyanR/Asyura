...
final class SimpleSiteService
{
    ...
    public function save(array $data): int
    {
        ...
        if ($name === '' || $url === '') {
            throw new \InvalidArgumentException('サイト名とサイトURLを確認してください。');
        }
        ...
        if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            throw new \InvalidArgumentException('管理メールアドレスが正しくありません。');
        }
        ...
        try {
            ...
            $stmt->execute([
                Security::randomToken(8),
                Security::randomToken(24),
                $name,
                $url,
                $rssUrl,
                $loginUrl,
    ...
    private static function optionalUrl(mixed $value, string $label): ?string
    {
        ...
        if ($url === '') {
            throw new \InvalidArgumentException($label . 'はhttp://またはhttps://から始まるURLを入力してください。');
        }
        return $url;
    }
