...
final class SimpleSiteService
{
    ...
    private function createDefaultWidgets(int $siteId, string $siteName): void
    {
        ...
        $stmt->execute([$siteId,Security::randomToken(8),'ranking','A',$siteName . ' 逆アクセスランキング',10,WidgetDesign::defaults('ranking')['template_html'],WidgetDesign::defaults('ranking')['custom_css']]);
        foreach (range('A','J') as $slot) $stmt->execute([$siteId,Security::randomToken(8),'links',$slot,"相互リンク {$slot}",20,'<a href="{url}" rel="{rel}" target="{target}">{title}</a>','.asyura-links{font-size:14px}']);
        foreach (range('A','J') as $slot) {
            ...
            $stmt->execute([$siteId,Security::randomToken(8),'rss','IMAGE-'.$slot,"画像RSS {$slot}",10,WidgetDesign::defaults('rss','IMAGE-'.$slot)['template_html'],WidgetDesign::defaults('rss','IMAGE-'.$slot)['custom_css']]);
